...
void insertSpaceIntoEditFromBeatRange (Edit& edit, BeatRange beatRange)
{
    ...
    const auto lengthInTimeToInsert = TimeDuration::fromSeconds (beatRange.getLength().inBeats() * tempoAtInsertionPoint.getApproxBeatLength().inBeats());
        ...
        if (auto track = first->getClipTrack())
        {
            if (auto newClip = track->insertMIDIClip (first->getName(), first->getPosition().time, nullptr))
            {
                ...
                for (auto c : clips)
                {
                    ...
                    auto offset = BeatDuration::fromSeconds (c->getPosition().getOffset().inSeconds() * c->edit.tempoSequence.getBeatsPerSecondAt (c->getPosition().getStart(), true));
