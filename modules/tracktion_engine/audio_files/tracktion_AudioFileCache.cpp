...
void AudioFileCache::setCacheSizeSamples (SampleCount samples)
{
    ...
    if (cacheSizeSamples != samples)
    {
        ...
        mapperThread->startThread (juce::Thread::Priority::highest);

        refresherThread.reset (new RefresherThread (*this));
        refresherThread->startThread (juce::Thread::Priority::highest);
    }
}
...
AudioFileCache::Reader::Ptr AudioFileCache::createReader (const AudioFile& file)
{
    ...
    {
        backgroundReaderThread.startThread (juce::Thread::Priority::highest);
