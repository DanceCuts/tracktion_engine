...
namespace AppFunctions
{
    ...
    {
        if (auto transport = getActiveTransport())
            transport->setLoopIn  (forceAtCursor ? TimePosition::fromSeconds (transport->position.get())
    ...
    {
        if (auto transport = getActiveTransport())
            transport->setLoopOut  (forceAtCursor ? TimePosition::fromSeconds (transport->position.get())
                                                  : getCurrentUIBehaviour().getEditingPosition (transport->edit));
    }
